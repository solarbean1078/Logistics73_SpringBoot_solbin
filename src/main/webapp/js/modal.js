...
const setMpsModal = () => {
    ...
    mpsGridOptions.onGridReady = function () {
        for (let i = 0; i < jsonda[0].gridRowJson.length; i++) {
            jsonda[0].gridRowJson[i].no = jsonda[0].gridRowJson[i].mpsPlanClassification == "수주상세"
                ? jsonda[0].gridRowJson[i].contractDetailNo : jsonda[0].gridRowJson[i].salesPlanNo;
        }
        console.log("######콘솔에 뜰까요?");
        mpsGridOptions.api.setRowData(jsonda[0].gridRowJson);
    };
...
const getMpsList = () => {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            console.log(txt);
            jsonData = JSON.parse(txt);
            jsonda.push(jsonData);
            console.log("$$$"+JSON.stringify(jsonda[0].gridRowJson));
            if (jsonData.errorCode != 1) {
                swal({
                    text: '데이터를 불러드리는데 오류가 발생했습니다.',
                    icon: 'error',
